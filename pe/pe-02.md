...
2) Add an HTML Checkbox that will "Shoutify" the text ([put it in all capital letters](https://www.w3schools.com/jsref/tryit.asp?filename=tryjsref_touppercase)) when the user clicks a button and the checkbox is checked
    ...
    - OR you could just "shoutify" the text, or not, in your `greet()` function, depending on the state of your checkbox 

3) Convert all of the functions in the app to [arrow functions](../notes/js-functions.md#vi-b-arrow-function-examples) - including all anonymous (unnamed) functions. Hint: search for the `function` keyword in your source code

4) Make sure that your HTML and JS is following our [IGME-330 class code standards](../notes/code-style-required-330.md)

    - [EXCEPT for ***ES6 Modules*** - don't worry about those - we'll cover that next time](../notes/code-style-required-330.md#iii-a-es6-modules)
    - [But be sure to eliminate ***Extraneous or Unnecessary Code***](../notes/code-style-required-330.md#iii-b-extraneous-or-unnecessary-code)
