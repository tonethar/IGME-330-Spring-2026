...
- [**HW-1 - Technobabble - Ultimate Version**](../hw/hw-1.md)
  ...
  - BTW - `"use strict"` is not needed with ES6 modules
    - nor is `window.onload = init;` - just call `init()` directly
...
- ***Other stuff*** - linking to **main.js** from you HTML file:
  - looks like this --> `<script type="module" src="src/app.js">` or this --> `<script type="module" src="./src/app.js">`
  - NOT this --> `<script type="module" src="/src/app.js">` (the `/` at the beginning of the path says "go to the top of the file system")
- Loading files (such as **babble.json**) with XHR:
  - paths (from **src/main.js** or anywhere) look like this --> `data/babble.json` or this --> `./data/babble.json`
  -  NOT this --> `../data/babble.json`
  -  BECAUSE, on the client-side, all file paths are relative to the HTML file the .js file is linked to


---

- ***&darr;&darr;&darr; NEW UNIT! Now let's talk about the canvas drawing API! &darr;&darr;&darr;***
...
- *Canvas Quiz* (see myCourses Assignments tab)
