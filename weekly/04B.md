...
- Test it - the ball should bounce off the top now
...
- Now let's write some code that moves the paddle so that it follows the mouse pointer's location in the window - put this code near the bottom, right before the call to `loop()`:
...
### I-D. Finish it up: paddle collision and left/right wall logic

- This will go in `update()`:
...
- You should now see that you are getting functional paddle/ball physics, where the ball angle is altered depending on what part of the paddle it strikes
- ***YOU DO -->*** write `// Wall Collisions (Left/Right)` so that we havecollisions working for everything (except the bottom of the screen)
- Bring it to class completed and ready to go necxt time, we'll be adding bricks, ball/bick collisions, scoring and sound
